...
int main () {
    ...
    // values are from 0 to N inclusive
    SegmentTree seg;
    seg.init(n + 1);

    // number of inversions (a > b) where i is the b value
    vector<ll> inversions (n + 1);
    for (int i = 0; i < n; i++) {
        seg.add(arr[i], arr[i], 1);
        ...
        */
        ...
        ll inv = seg.sum(arr[i] + 1, n);

        print(inv);

        inversions[arr[i]] += inv;
    }
