...
struct SegTree {
    ...
    // find position of the kth appearance of 1
...
int main () {
    ...
    for (int i = 0; i < q; i++) {
        cin >> queries[i].first; queries[i].first -= n;
        queries[i].second = i;
    }
    ...
    for (int i = 1; i < arr.size(); i++) {
        ...
        while (qidx < queries.size() && queries[qidx].first <= curT) {
            // finds the offset of the query in the current block
            // the first year is prevT + 1, second is prevT + 2, it's 1 indexed
            ll index = (queries[qidx].first - prevT - 1) % active_cities;

            // +1 because seg tree is tracking sum, you want position of sum = offset
            // another +1 to convert seg tree 0 index
            ll res = seg.findk(index + 1) + 1;
            ans[queries[qidx].second] = res;

            print(queries[qidx].first, index, res);
            qidx++;
        }
    ...
    // everybody is caught up now, ez cycle for the rest

    for (int j = 0; j < idx[arr[arr.size() - 1].first].size(); j++) {
        seg.add(idx[arr[arr.size() - 1].first][j] - 1, idx[arr[arr.size() - 1].first][j] - 1, 1);
        active_cities++;
    }
    ...
    while (qidx < queries.size()) {
        ll offset = (queries[qidx].first - curT - 1) % active_cities;
        ll res = seg.findk(offset + 1) + 1;
        ans[queries[qidx].second] = res;
        print(queries[qidx].first, offset, res);
        qidx++;
    }
