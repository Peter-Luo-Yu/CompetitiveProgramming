...
/*
 * tries to scavage free blocks from the child of a region
 * if the region has smaller than min_blocks free regions
 */
...
int rebalance_free_blocks(region_t *root, size_t min_blocks) {
  ...
  if (cur_blocks < (int) min_blocks) {
    ...
    int left_free = -1;
    if (root->left != NULL) {
      left_free = count_free_blocks(root->left);
    }

    int right_free = -1;
    if (root->right != NULL) {
      right_free = count_free_blocks(root->right);
    }
...
} /* rebalance_free_blocks() */

/*
 * returns the sum of all block sizes in BST
 */
...
/*
 * helper function that searches the entire BST and finds the
 * size of the smallest free block that still has a size larger 
 * than the specificed size
 */
...
} /* find_best_region_size() */

/*
 * helper function that finds the first region
 * with a free block of the specified size
 */
...
/*
 * helper function that inserts a free block into a region
 */
...
/*
 * the function de-allocates the entire BST
 */
