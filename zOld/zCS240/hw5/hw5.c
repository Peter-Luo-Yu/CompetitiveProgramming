...
void printjob(job_t job) {
  printf("%s %s %d %d %d %d %s %d\n", job.id, job.user_id, job.required_cores, job.required_time, job.priority_level, job.status, job.dependency_job_id, job.submission_time);
} */
...
job_t read_job(FILE *fp, int n) {
  ...
  //printf("n: %d | %s %s %d %d %d %d %s %d\n", n, job.id, job.user_id, job.required_cores, job.required_time, job.priority_level, job.status, job.dependency_job_id, job.submission_time);
...
int update_job_status(FILE *fp, int n) {
  ...
  if (independent_job == 0) {
    ...
    if ((found == 1) && ((dependency.status == WAITING) || (dependency.status == RUNNING))) {
      //printf("found\n");
...
int clean_job_queue(FILE *fp, int n) {
  ...
  for (int i = 0; i < end; i += sizeof(job_t)) {
    ...
    if ((cur.status == COMPLETED) || 
        ((cur.status == RUNNING) && (cur.submission_time + cur.required_time <= n))) {
      
      if (cur.status == COMPLETED) {
        //printf("expected: %d | n: %d\n", cur.submission_time, n); 
      } 
      ...
      for (int j = 0; j < end; j += sizeof(job_t)) {
        ...
        if ((depend.status != EMPTY) && (strcmp(depend.dependency_job_id, cur.id) == 0)) {
      ...
      //printf("NOT deleted | status: %d | submission: %d | required: %d | sum: %d\n", cur.status, cur.submission_time, cur.required_time, cur.submission_time + cur.required_time); 
    }
  ...
  for (int i = 0; i < end; i += sizeof(job_t)) {
    ...
    if (independent == 0) {
      ...
      for (int j = 0; j < end; j += sizeof(job_t)) {
        ...
        if ((strcmp(cur.dependency_job_id, dependency.id) == 0) && dependency.status != EMPTY) {
          dependency_found = 1;
